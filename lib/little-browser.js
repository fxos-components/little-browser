 ...
 * @type {Function}
 */
var debug = 1 ? console.log.bind(console, '[little-browser]') : function() {};
...
Private.prototype = {
  ...
  navigate: function(url, options) {
    debug('navigate', url, options, this.current);
    if (url == null) return;
    if (this.current && url === this.current.url) return;
  ...
  load: function(frame, from) {
    ...
    this.transitionFrame(frame.el, from, 0, { duration: duration });
  ...
  transitionFrame: function(el, from, to, options) {
